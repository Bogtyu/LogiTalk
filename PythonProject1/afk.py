...
class MainWindow(CTk):
   def __init__(self):
       ...
       # Основне поле чату
       self.chat_field = CTkScrollableFrame(self, fg_color="black")
       ...
       self.bg_image = CTkImage(light_image=self.bg, size=(50, 40))

       self.send_button = CTkButton(self, text='', width=5, height=4, command=self.send_message, image=self.bg_image, fg_color="blue")
       ...
       self.add_message("Демонстрація відображення зображення:",
                        CTkImage(Image.open('bg.png'), size=(300, 300)))

       try:
           self.sock = socket(AF_INET, SOCK_STREAM)
           self.sock.connect(('4.tcp.ngrok.io', 18087))
